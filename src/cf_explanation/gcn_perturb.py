...
from src.utils.utils import get_degree_matrix, normalize_adj, create_symm_matrix_from_vec, create_vec_from_symm_matrix
from src.gcn import GraphConvolution, GCNSynthetic, GCN2Layer
...
class GCN1LayerPerturb(nn.Module):
	"""
	1-layer GCN used in GNN Explainer
	"""
	def __init__(self, nfeat, nhid, nout, nclass, adj, dropout, beta, edge_additions=False):
		super(GCN1LayerPerturb, self).__init__()
		self.adj = adj
		self.nclass = nclass
		self.beta = beta
...
class GCN4LayerPerturb(nn.Module):
	"""
	4-layer GCN used in GNN Explainer
	"""
	def __init__(self, nfeat, nhid, nout, nclass, adj, dropout, beta, edge_additions=False):
		super(GCN4LayerPerturb, self).__init__()
		self.adj = adj
		self.nclass = nclass
		self.beta = beta
		self.num_nodes = self.adj.shape[0]
	...
	def forward(self, x, sub_adj):
		...
		x3 = F.relu(self.gc3(x2, norm_adj))
		x4 = self.gc4(x3, norm_adj)
		x = self.lin(x4)
		x = F.dropout(x, self.dropout, training=self.training)
		return F.log_softmax(x, dim=1)
